...
│   ├── fidelity-and-mapping.md       # 转换模式 + 内容保真门 + 源章节映射（含压缩预算/重复判定/交付报告）
...
**认知降维 ≠ 信息丢失**。写 HTML 前先选转换模式（保真 / 双层默认 / 摘要），并给源章节打 6 类保真标签（核心需求 100% 保留、执行规格折叠保全文、重复表达合并而非删除）。交付时附源章节映射表 + 5 行报告，自证没有无意遗漏。这条源自一次真实复盘：同一份规格被摘要型执行压成一半，丢失了研发落地必需的字段/状态/异常。

详见 `references/fidelity-and-mapping.md`。
...
10. **转换模式 + 内容保真门**（曾把"认知降维"执行成摘要型，把 ~13,000 字符规格压到 7,400，丢失字段/状态/异常——根因是没设信息完整度下限。对照 v15 保真型 109KB/121 卡片 vs v16 摘要型 48KB/37 卡片后，确立"双层型为默认、保真门守完整度、源章节映射自证无遗漏"）
