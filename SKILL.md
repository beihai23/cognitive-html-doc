...
| 内容类型 | 表现形式 |
|---|---|
| 关键数字（KPI、QPS、timeout、规模、阈值、版本号、SLO） | **关键数字卡片**（脱离表格，用 `tabular-nums`） |
...
12. **用特定场景词命名通用原则**（元反模式）：把"关键数字应该突出"写成"KPI 应该突出"——技术文档读者误以为"我没有 KPI 就不需要这种呈现"。其他例子：把"分层"等同于"折叠"、把"信息架构"等同于"产品菜单"。原则要抽象到场景无关，例子才落地说场景
