...
| 内容类型 | 表现形式 | 多场景例子 |
|---|---|---|
| 关键数字 | **关键数字卡片**（脱离表格，用 `tabular-nums`） | 产品 KPI 北极星 / 技术 SLO 阈值 / API 限流值 / 操作手册的耗时上限 |
| 核心约束 / 警告 | **callout**（左 4px 色条 + 浅色背景，前置到章节开头） | 产品红线 / 技术硬性限制 / API 鉴权要求 / 操作手册的危险操作 |
| 系统架构 | **Mermaid graph** + subgraph 分层 + classDef 颜色编码 | 产品双渠道架构 / 技术微服务拓扑 / API 网关路由 |
| 业务流程 | **Mermaid flowchart** + 决策分支色分 | 产品用户决策 / 技术 API 请求处理 / 操作手册故障排查 |
| 时序交互 | **Mermaid sequenceDiagram** + autonumber + Note | 产品用户-bot 交互 / 技术 API 调用链 / 操作手册部署步骤 |
| 状态转换 | **Mermaid stateDiagram-v2** | 产品订阅状态 / 技术任务状态机 / API 资源生命周期 |
| 时间线 / 阶段演进 | **Mermaid timeline** | 产品 Phase 路径 / 技术版本演进 / API 弃用时间表 |
| 多方案对比 | **并排卡片**（不同颜色边框）+ Tab 切换 | 产品双人群 / 技术方案对比 / API 多版本 |
| 长列表 / 复杂层级 | **嵌套卡片网格** 或 **SVG 自定义可视化**（金字塔、Venn 图） | 产品文档体系 / 技术模块依赖 / API 字段层级 |
| 详细参数 / 配置 | **响应式表格**（sticky header、斑马线、hover 高亮） | 产品能力矩阵 / 技术配置项 / API 字段定义 |
| 术语 | 折叠到附录，正文用 `<abbr>` 或 tooltip 内联 | 产品业务术语 / 技术缩写 / API 错误码 |
